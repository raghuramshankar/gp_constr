...
def is_symPD_svd(M, verbatim=False, sym_tol=1e-5, psd_tol=1e-5):
    ...
    if verbatim:
        if is_sym:
            ...
            print(
                "WARNING! Symmetric: error = {} > {}, NOT OK".format(symdiff, sym_tol)
            )

        print(
            "Eigenvalues real part: min = {}, max = {}".format(
                eig_real_min, eig_real_max
            )
        )
        print(
            "Eigenvalues imag part: min = {}, max = {}".format(
                eig_imag_min, eig_imag_max
            )
        )

        if is_psd:
            print(
                "Postive definite, M - VSV.T where M = USV.T (SVD): error = {} < {}, OK".format(
                    svddiff, psd_tol
                )
            )
        else:
            print(
                "WARNING! Postive definite, M - VSV.T where M = USV.T (SVD): error = {} > {}, NOT OK".format(
                    svddiff, psd_tol
                )
            )
...
def traceprod(A, B):
    ...
    return np.sum(np.inner(np.array(A), np.array(B).T))
